...
getpieces(){
    ...
    //     this.connection()
    //     this.getTime()
    //     return ({machID:this.machID,time:this.time ,pieces:this.pieces})
    // }
       ...
       this.io.on('connection', (socket) => {
            socket.on(`subscribeToTimer+${this.machID}`, (interval) => {
                ...
                setInterval(() => {
                    this.getpieces()
                    this.getTime()
                    socket.emit(this.machID, this.pieces, this.time);
